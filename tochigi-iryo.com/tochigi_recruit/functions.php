...
//=== カスタム投稿 ==== 
if (0):
CRPostManager::getInstance()
	->register_custom("post", array(
			"image01" => array(
				'type' => 'image',
				'label' => '画像（一覧表示用）',
				'desc' => '一覧表示用の画像です。347x203推奨',
				
				"size_key" => "post_image01",
			),
			"pickup" => array(
				"label" => 'Pickup',
				"desc" => "数値の大きいものから最新の３件が表示されます。",
			...
			"pv" => array(
				"label" => 'PV',
				"desc" => "編集することも可能ですが、自動的にカウントされていきます。",
				"type"=> "text", // text, textarea
				"placeholder" => "0",
				"default_value" => "0",
				"formatting" => "html", // html
...
add_image_size("post_image01", 347, 203,true);
add_image_size("post_image02", 237, 183,true);
add_image_size("post_image03", 287, 188,true);
endif;
...
function my_login_logo() { ?>
	<style>
		.login #login h1 a {
			width: 310px;
			height: 70px;
			background: url(<?php echo get_stylesheet_directory_uri(); ?>/images/loginlogo.png?2) no-repeat 0 0;
		}
	</style>
	<script>
		setTimeout(function() {
...
/*
//=== ブログのPVカウント
add_action("wp_head", "blog_inc");
function blog_inc($s) {
	if (is_singular("post"))  {
		$count = (int)get_field("pv");
		++$count;
		
		$field_key = null;
		
		$options = CRPostManager::getInstance()->acf_field_groups;
		foreach($options as $_options) {
			foreach($_options["fields"] as $_field) {
				if ($_field["name"] == "pv") {
		...
		if ($field_key)
			update_field($field_key , $count);
		//update_field("pv" , $count);
	}
}*/
//=== 検索はブログ専用とする
add_action( 'pre_get_posts', 'my_recruit_query_blog' ); 

function my_recruit_query_blog( $query ) {
