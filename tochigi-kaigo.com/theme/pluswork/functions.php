  ...
  public function posts_clauses_order_item($pieces, WP_Query $query)
  {
    ...
    if ( ! is_admin() && is_archive() ) { # 非管理画面＆アーカイブ表示時のみ適用
      ...
      if ( $query->is_tax("genre") ) {
        $is_course = true;
      } else if ( $query->get("post_type") == "item" ) {
        $is_course = true;
      }
      ...
      if ( $is_course ) {
        ...
        } else if ( $ref == "recommend" ) { # 「おすすめ」のみの場合、おすすめ優先順位でソート強制
        ...
        } else if ( $query->get("orderby") == "" ) { # その他でソート順が指定されていない場合、更新日付でソート
...
register_nav_menu('sp_header', 'スマートフォンメニュー');
register_nav_menu('sp_footer', 'スマートフォン-フッターメニュー');
...
if ( ! is_admin() ) {
  function register_script()
  {
    ...
    wp_register_style('fontawesome', get_bloginfo('stylesheet_directory') . '/css/font-awesome.min.css');
  }
